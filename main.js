...
Matrix.prototype.buildMatrix = async function(){
    ...
    if(window.innerWidth < 600){
        ...
        this.rows = Math.floor((window.innerHeight-170)/this.blockSize);
    }
    else{
        this.blockSize = 22;
        this.cols = Math.floor((window.innerWidth-80)/this.blockSize);
        this.rows = Math.floor((window.innerHeight-115)/this.blockSize);    
    }
...
Matrix.prototype.reset = async function() {
    let start = this.blocks[this.start_block[0]].childNodes[this.start_block[1]];
    if(this.start_block[0] !== undefined && start.style.backgroundColor === explore || start.style.backgroundColor === visited || this.start_block[0] !== undefined && start.style.backgroundColor === explore || start.style.backgroundColor === path){
        for(let i=0; i<this.rows; ++i){
            for(let j=0; j<this.cols; ++j){
                this.blocks[i].childNodes[j].innerHTML = "";
                if(this.blocks[i].childNodes[j].style.backgroundColor !== wall && this.blocks[i].childNodes[j].style.backgroundColor!=="white"){
...
Matrix.prototype.findPath = async function() {
    ...
    if(this.start_block[0] !== undefined && this.end_block[0] !== undefined){
        if(this.blocks[this.start_block[0]].childNodes[this.start_block[1]].style.backgroundColor === path || this.blocks[this.start_block[0]].childNodes[this.start_block[1]].style.backgroundColor === visited){
            this.reset();
        }
        switch(algo){
            case 'bestFirst':
            ...
            case 'dijkstra':
                ...
                break; 
            case "bidirectional":
                await search.bidirectionalSearch(this.blocks, this.start_block, this.end_block);
                break;
            case "astar":
...
Matrix.prototype.generateMaze = async function(){
    ...
    switch(select_maze){
        case "rd":
            await maze.recursiveDivisionMaze(this.blocks, 1, this.rows-2, 1, this.cols-2);
            break;
        ...
        case "vb":
            await maze.verticalBarsMaze(this.blocks, 1,this.rows-2, 1, this.cols-2);
            break;

        case "hb":
            await maze.horizontalBarsMaze(this.blocks, 1, this.rows - 2, 1, this.cols - 2);
            break;
